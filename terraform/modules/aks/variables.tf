...
# Real AAD-integrated Kubernetes RBAC. Deliberately no default for the
# admin group list -- see this module's main.tf comment on
# `azure_active_directory_role_based_access_control`: an empty list here is
# a REAL, valid, and safe choice (AAD auth enabled, nobody has cluster-admin
# via it yet), but it must be an explicit choice a caller makes, not an
# accidental one from a default nobody thought about. Every environment
# wiring this module in must pass a real value, even if that value is `[]`.
...
variable "azure_rbac_enabled" {
  description = <<-EOT
    ...
    group object id>"}]`) -- this is the mode this module's RBAC block and
    `infra/k8s/base/rbac/`'s manifests are written for.
