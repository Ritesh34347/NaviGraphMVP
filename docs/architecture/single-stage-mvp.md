...
| Client-facing surfaces (chat UI, MCP tool-surface server, Slack bot) | **Real, no live Slack/Claude Desktop verification** | Chat UI: `tsc`/`next lint`/`next build` clean, 6 Playwright tests against a real `next start` server. MCP server: 11 unit tests plus a real stdio-transport handshake proof. Slack bot: 25 unit tests including adversarial signature coverage. None has a real per-caller `tenant_id` mapping yet (`LIMITATIONS.md` item 63) |
| Admin & governance surfaces (lineage search, admin CLI/UI, AAD K8s RBAC) | **Real code; RBAC never applied to a live cluster** | Lineage search verified against a real Postgres instance end to end (query + agent-runtime route + gateway proxy); admin CLI exercised for real against real `DataSource`/lineage rows; admin UI has 3 Playwright tests. AAD K8s RBAC Terraform/K8s code is real (`terraform fmt`/`kustomize build` clean) but deliberately not applied -- see `docs/runbooks/aad-k8s-rbac-rollout.md` (`LIMITATIONS.md` item 64) |
