...
export const env = {
  ...
  // Phase 14 note: the chat UI (src/app/chat) has no real signed-in tenant
  // to read yet -- NextAuth's Azure AD wiring above verifies a user's
  // identity, not which NaviGraph business tenant they belong to (see
  // route.ts's own Phase 1 note). Until that mapping exists, the chat UI
  // sends this fixed tenant_id with every request rather than inventing an
  // ad hoc per-browser value. Defaults to the same demo tenant used
  // throughout this repo's own eval/integration fixtures.
