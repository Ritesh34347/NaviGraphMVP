...
back to `PostgresSettings`'s own global `SOURCE_POSTGRES_*` env-var
reading.
...
def build_postgres_settings(
    ...
    return PostgresSettings(
        source_postgres_host=field("host"),
        source_postgres_port=int(port) if port else 5432,
        source_postgres_database=field("database"),
        source_postgres_user=field("user"),
        source_postgres_password=field("password"),
        source_postgres_sslmode=field("sslmode", "prefer"),
    )
