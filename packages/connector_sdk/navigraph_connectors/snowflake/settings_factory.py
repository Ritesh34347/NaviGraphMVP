...
`{"secret_scope": "navikenz_poc_snowflake"}`. This REPLACES the older,
pre-item-21 `{"env_prefix": "SNOWFLAKE"}` shape (a single, global prefix
shared by every Snowflake `DataSource` in the process); real `DataSource`
rows created before this change need their `connection_ref` re-registered
under the new shape -- see DECISIONS.md.
"""
